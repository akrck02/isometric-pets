...
static func handle_touch(event : InputEventScreenTouch, delta : float):
	...
	data.delta = delta
	data.touch_data = TouchInputData.from(event.index, Inputs.Type.Tap, touch_points)
	...
	# Screen pinch
		...
		data.touch_data.type = Inputs.Type.Pinch
	...
	# Three finger touch
		...
		data.touch_data.type = Inputs.Type.ThreeFingerTap
...
static func handle_drag(event : InputEventScreenDrag, delta : float):
	...
	match touch_points.size():
		1:  
			data.touch_data.type = Inputs.Type.DragMove
			data.touch_data.relative = event.relative
			InputManager.movement_requested.emit(data)
		2:  
			data.touch_data.type = Inputs.Type.DragPinch
